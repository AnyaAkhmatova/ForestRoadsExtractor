...
class ForestRoadsExtractor:
    ...
    def __init__(self, model_name, model_path, device=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')):
        ...
        if model_name == 'fcn_resnet50':
            self.model = models.segmentation.fcn_resnet50(num_classes=2).to(self.device)
        elif model_name == 'deeplabv3_resnet50':
            self.model = models.segmentation.deeplabv3_resnet50(num_classes=2).to(self.device)
        else:
            self.model = UNet(3, 2, dropout=0.2).to(self.device)
